...
{
P2DIR &= ~(CC1101_GDO0 + CC1101_GDO2);
P2IES &= ~(CC1101_GDO0 + CC1101_GDO2);         // Int on rising edge: GDO0 interrupts when receives sync word
P2IFG = 0;                                                                 // Clear flags
P2IE |= CC1101_GDO0 + CC1101_GDO2;             // Enable ints 
...
while(1)
  ...
  if(e & CC1101_EV_RX_SYNC)
  {
      ...
      PktLen = 0;
      P2IE &= ~(CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2);                         // Disable Port 2 interrupts
      P2IES |= (CC1101_GDO0);                                                                   // Change GDO0 interrupt to trigger on falling edge so the end of the packet can be detected 
      P2IFG &= ~CC1101_GDO0;                                                                    // Clear flags
      P2IE |= CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2;                            // Enable Port 2 interrupts
  }
  ...
  {         
   ...
   // This handles the case where the packet size is less than the RX FIFO threshold (RxBytesThr variable), since the packet size isn't large enough to trigger the
   // GDO2 interrupt.
   if (small_packet)
   {
   ...
   // The default case, for packets > RxBytesThr.
   else
    {
   RxFIFOLen = (Radio_Read_Status(TI_CCxxx0_RXBYTES, CC1101) & TI_CCxxx0_NUM_RXBYTES);
   if (RxFIFOLen > 0){
   ...
   state = IDLE;
   P2IE &= ~(CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2);                                              // Disable Port 2 interrupts
   P2IES &= ~(CC1101_GDO0);                                                                  // Change GDO0 edge back to rising to prepare for next sync word
   P2IFG &= ~CC1101_GDO0;                                                                    // Clear flags
   P2IE |= CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2;                                                 // Enable Port 2 interrupts
    }
   }
   ...
   if(e & CC1101_EV_TX_START)
   {
          ...
          P2IES |= (CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2);                                              // Change edge interrupt happens on
          P2IFG = 0;                                                                         // Clear flags
          P2IE |= CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2;                                                 // Enable Port 2 interrupts
 ...
 if(e & CC1101_EV_TX_THR)
 {
  ...
  if(e & CC1101_EV_TX_END)
  {
   ...
   P2IES &= ~(CC1101_GDO0 + CC1101_GDO2);                                                           // Change edge interrupt happens on
   P2IFG = 0;                                                                         // Clear flags        
   P2IE |= CC1101_GDO0 + CC1101_GDO2 + CC2500_GDO0 + CC2500_GDO2;           
...
{
if (radio){
Radio_Write_Registers(TI_CCxxx0_IOCFG0,   0x3F, 1);   // Asserts when sync word has been sent / received, and de-asserts at the end of the packet. 
...
Radio_Write_Registers(TI_CCxxx0_MDMCFG4,  0xF5, 1);   // F5 = 1200 baud, F8 = 9600 baud
...
Radio_Write_Registers(TI_CCxxx0_SYNC1,    0x7E, 1);
}
