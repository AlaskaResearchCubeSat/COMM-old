...
void Radio_Write_Burst_Registers(char addr, char *buffer, char size, char uhf)
{
  unsigned int i;
...
void Radio_Read_Burst_Registers(char addr, char *buffer, char count, char uhf)
{
  char i;
  if (uhf)
     P5OUT &= ~CS_1101;                          // CS enable CC1101
  ...
  while (!(UC1IFG & UCB1TXIFG));                 // Wait for TXBUF ready
  UCB1TXBUF = (addr | TI_CCxxx0_READ_BURST);  // Adding 0xC0 to address tells the radio to read a single byte
  while (UCB1STAT & UCBUSY);                  // Wait for TX to complete
...
Radio_Write_Registers(TI_CCxxx0_PKTCTRL0, 0x04, 0);  // Packet automation control.
...
char RF_Receive_Packet(char *rxBuffer, char *length, char uhf)
{
  char status[2];
  char pktLen;
  ...
  if ((Radio_Read_Status(TI_CCxxx0_RXBYTES, uhf) & TI_CCxxx0_NUM_RXBYTES))
  {
    pktLen = Radio_Read_Registers(TI_CCxxx0_RXFIFO, uhf); // Read length byte

    if (pktLen <= *length)                  // If pktLen size <= rxBuffer
    {
      Radio_Read_Burst_Registers(TI_CCxxx0_RXFIFO, rxBuffer, pktLen, uhf); // Pull data
...
void RF_Send_Packet(char *txBuffer, char size, char uhf)
{
