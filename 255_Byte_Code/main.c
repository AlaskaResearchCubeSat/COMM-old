...
{                                           
  ...
  //2.2 -- GDO0  CC2500
  //2.3 -- GDO2  CC2500
  P2DIR &= ~(BIT0 + BIT1);  // + BIT2 + BIT3  These interrupts are giving me problems. Removed temporarily.                                   
  P2IES &= ~(BIT0 + BIT1);    // Low-to-high transition for interrupts
  P2IFG = 0;                                // Clear flags
  P2IE |= BIT0 + BIT1;        // Enable ints
