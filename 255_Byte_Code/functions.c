...
while(1)
  ...
  {         
   ...
   state = IDLE;
   P2IE &= ~(BIT0 + BIT1 + BIT2 + BIT3);                                              // Disable Port 2 interrupts
   P2IES &= ~(BIT0);                                                                  // Change GDO0 edge back to rising to prepare for next sync word
   P2IFG &= ~BIT0;                                                                    // Clear flags
   P2IE |= BIT0 + BIT1 + BIT2 + BIT3;                                                 // Enable Port 2 interrupts
    }
   }
...
Radio_Write_Registers(TI_CCxxx0_FREQ1,    0xA7, 1);   // 10, A7, 62 = 433 MHz;  10, C4, EC = 436 MHz 
